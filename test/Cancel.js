...
import * as requestBody from '../CancelledGame/CancelRequests.js';
import * as body from '../CancelledGame/CancelBody.js';
import * as fetchFunction from '../CancelledGame/CancelFunctions.js';
...
describe('final_settlement_request - Checks if response status is successful', async () => {
  ...
  before(async () => {
    ...
    console.log('Payout is:', global.payyof);
...
describe('get_balance_request - Checks balance and if response status is successful', async () => {
  ...
  before(async () => {
    ...
    testResponseBody = await testResponse.json();
    currentBalance = testResponseBody.balances[0].amount;
    initialBalance = currentBalance - global.payyof + global.bet + global.bet;
    expectedBalance = initialBalance - global.bet - global.bet + global.payyof;
